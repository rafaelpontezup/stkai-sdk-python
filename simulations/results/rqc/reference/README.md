# Rate Limiting Simulation Results (RQC)
...
> **Note: RQC-Specific Reference Graphs**
>
> These reference graphs are for **RQC (Remote Quick Commands)** workloads:
>
> | Workload | POST Latency | Reference Graphs |
> |----------|--------------|------------------|
> | **RQC** | ~200ms (submit) + polling | Here (`rqc/reference/`) |
> | **Agent::chat()** | ~15s (LLM processing) | See `agent/reference/` |
>
> **Why it matters**: Agent has much longer request times (~15s vs ~200ms), so natural throughput is limited by latency rather than rate limiting. The preset recommendations in this document are optimized for RQC; see Agent reference for Agent-specific findings.
...
```bash
./scripts/run_simulations.sh rqc
```

Results will be saved to `simulations/results/rqc/latest/`.

To update reference graphs:
```bash
DATE=$(date +%Y-%m-%d)
for f in simulations/results/rqc/latest/graph_*.png; do
  cp "$f" "simulations/results/rqc/reference/${DATE}_$(basename $f)"
done
```
