"""Tests for Agent Result Handlers."""
...
def make_context(raw_result: Any, handled: bool = False) -> ChatResultContext:
    """Helper to create a valid ChatResultContext for testing."""
    request = ChatRequest(user_prompt="Test prompt", id="test-req-id")
    return ChatResultContext(request=request, raw_result=raw_result, handled=handled)
...
class TestRawResultHandler(unittest.TestCase):
    ...
    def test_default_result_handler_is_raw_handler(self):
        """DEFAULT_RESULT_HANDLER constant should be a RawResultHandler instance."""
        self.assertIsInstance(DEFAULT_RESULT_HANDLER, RawResultHandler)
...
class TestJsonResultHandler(unittest.TestCase):
    ...
    def test_returns_empty_string_for_empty_string(self):
        """Should return empty string when raw_result is an empty string."""
    ...
    def test_json_result_handler_singleton_exists(self):
        """JSON_RESULT_HANDLER constant should be a JsonResultHandler instance."""
        self.assertIsInstance(JSON_RESULT_HANDLER, JsonResultHandler)
...
class TestJsonResultHandlerChainWith(unittest.TestCase):
    ...
    def test_chain_with_creates_chained_handler(self):
        ...
        other_handler = Mock(spec=ChatResultHandler)
...
class TestChainedResultHandler(unittest.TestCase):
    ...
    def test_executes_handlers_in_sequence(self):
        ...
        class AddPrefixHandler(ChatResultHandler):
            def handle_result(self, context: ChatResultContext) -> Any:
                return f"prefix_{context.raw_result}"

        class AddSuffixHandler(ChatResultHandler):
            def handle_result(self, context: ChatResultContext) -> Any:
    ...
    def test_json_then_custom_transformation(self):
        ...
        class ExtractFieldHandler(ChatResultHandler):
            def handle_result(self, context: ChatResultContext) -> Any:
    ...
    def test_updates_handled_flag_after_first_handler(self):
        ...
        class TrackingHandler(ChatResultHandler):
            def handle_result(self, ctx: ChatResultContext) -> Any:
...
class TestChainedResultHandlerOf(unittest.TestCase):
    ...
    def test_of_multiple_handlers_executes_in_order(self):
        ...
        class DoubleHandler(ChatResultHandler):
            def handle_result(self, context: ChatResultContext) -> Any:
...
# ======================
# ChatResultContext Tests
# ======================

class TestChatResultContext(unittest.TestCase):
    """Tests for ChatResultContext."""

    def test_creates_context_with_required_fields(self):
        """Should create context with request and raw_result."""
        request = ChatRequest(user_prompt="Hello", id="req-123")
        context = ChatResultContext(request=request, raw_result="response")

        self.assertEqual(context.request, request)
        self.assertEqual(context.raw_result, "response")
        self.assertFalse(context.handled)

    def test_request_id_property(self):
        """Should return request ID from request_id property."""
        request = ChatRequest(user_prompt="Hello", id="my-request-id")
        context = ChatResultContext(request=request, raw_result="response")

        self.assertEqual(context.request_id, "my-request-id")

    def test_raises_assertion_for_empty_request(self):
        """Should raise AssertionError if request is None."""
        with self.assertRaises(AssertionError):
            ChatResultContext(request=None, raw_result="response")  # type: ignore
    ...
    def test_is_frozen_dataclass(self):
        ...
        with self.assertRaises(AttributeError):
            context.handled = True  # type: ignore
